...
object WithColumn {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df2 =
      spark.createDataFrame(spark.sparkContext.parallelize(dataRows), schema)
    ...
    spark
      .sql(
        "SELECT salary*100 as salary, salary*-1 as CopiedColumn, 'USA' as country FROM PERSON"
      )
      .show(true)
